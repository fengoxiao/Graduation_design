...
import re
import time
from crawler.database_test_total import economics_create_interface
# 清理html标签
def clean_tag(string):
    ...
    return dd
count=0
url_start='http://www.hellosea.net/Economics/1/index_{}.html'
for page in range(1,145):
    try:
        if page==1:
            url='http://www.hellosea.net/Economics/1/'
        else:
        ...
        for news in news_list:
            ...
            # print(news_web_url)
            # print(news_source)
            if economics_create_interface(news_title,'蓝色经济',news_date,news_summary,news_content,news_web_url,news_source):
                count+=1
