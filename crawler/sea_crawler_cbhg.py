...
import re
import time
from crawler.database_test_total import cbhg_create_interface,cbhg_retrieve_interface
# 清理html标签
def clean_tag(string):
    ...
    return dd
count=0
url_start='http://www.hellosea.net/cbhg/news/index_{}.html'
#http://www.hellosea.net/Edu/1/index_60.html
for page in range(1,28):
    try:
        if page==1:
            url='http://www.hellosea.net/cbhg/news/'
        else:
        ...
        for news in news_list:
            ...
            #新方法
            if not cbhg_retrieve_interface(news_web_url):
                continue
            ...
            if not news_content:
                continue
            if cbhg_create_interface(news_title,'船舶海工',news_date,news_summary,news_content,news_web_url,news_source):
                count+=1
...
count=0
url_start='http://www.hellosea.net/cbhg/news/1/index_{}.html'
#http://www.hellosea.net/Edu/1/index_60.html
for page in range(1,23):
    try:
        if page==1:
            url='http://www.hellosea.net/cbhg/news/1/'
        else:
            url = url_start.format(page)
        response = requests.get(url)
        string = response.text
