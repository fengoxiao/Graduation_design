...
import re
import time
from crawler.database_test_total import mil_create_interface,mil_retrieve_interface
# 清理html标签
def clean_tag(string):
    ...
    return dd
count=0
url_start='http://www.hellosea.net/Mil/1/index_{}.html'
for page in range(1,66):
    try:
        if page==1:
            url='http://www.hellosea.net/Mil/1/'
        else:
        ...
        for news in news_list:
            ...
            #新方法
            if not mil_retrieve_interface(news_web_url):
                continue
            ...
            if not news_content:
                continue
            if mil_create_interface(news_title,'海洋军事',news_date,news_summary,news_content,news_web_url,news_source):
                count+=1
...
count=0
url_start='http://www.hellosea.net/Mil/4/index_{}.html'
for page in range(1,66):
    try:
        if page==1:
            url='http://www.hellosea.net/Mil/4/'
        else:
            url = url_start.format(page)
        response = requests.get(url)
        string = response.text
