...
import re
import time
from crawler.database_test_total import edu_create_interface
# 清理html标签
def clean_tag(string):
    ...
    return dd
count=0
url_start='http://www.hellosea.net/Edu/1/index_{}.html'
#http://www.hellosea.net/Edu/1/index_60.html
for page in range(1,61):
    try:
        if page==1:
            url='http://www.hellosea.net/Edu/1/'
        else:
        ...
        for news in news_list:
            ...
            # print(news_web_url)
            # print(news_source)
            if edu_create_interface(news_title,'教育资讯',news_date,news_summary,news_content,news_web_url,news_source):
                count+=1
...
count=0
url_start='http://www.hellosea.net/Edu/2/index_{}.html'
#http://www.hellosea.net/Edu/1/index_60.html
for page in range(1,61):
    try:
        if page==1:
            url='http://www.hellosea.net/Edu/2/'
        else:
            url = url_start.format(page)
        response = requests.get(url)
        string = response.text
        ...
        for news in news_list:
            ...
            # print(news_web_url)
            # print(news_source)
            if edu_create_interface(news_title,'海洋高校',news_date,news_summary,news_content,news_web_url,news_source):
                count+=1
            print('第{}个新闻'.format(count))
            time.sleep(5)
    except:
